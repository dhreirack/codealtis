...
if(life_inv_kidney >= 2) exitWith {hint "Vous ne pouvez pas conserver plus d'organes!"};//if you already have 2 kidneys, then go sell them already, no stockpiling
if(!([true,"Organe",1] call life_fnc_handleInv)) exitWith {hint "Vous n'avez pas assez de place!"};//if no room for kidney, you cannot take their kidney, duh, waste not want not
