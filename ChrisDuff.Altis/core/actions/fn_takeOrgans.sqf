...
if(life_inv_kidney >= 2) exitWith {hint "Vous ne pouvez pas avoir besoin que beaucoup d'organes!"};//if you already have 2 kidneys, then go sell them already, no stockpiling
if(!([true,"kidney",1] call life_fnc_handleInv)) exitWith {hint "Vous ne disposez pas de place pour stocker tous les organes!"};//if no room for kidney, you cannot take their kidney, duh, waste not want not
